...
use crate::winnow_parser::util::is_space;
...
mod test {
    ...
    #[test]
    fn test_field_name() -> Result<()> {
        let mut p = StreamParser::new(&b"Content-Type: 3\r\n"[..]);
    ...
    #[test]
    fn test_field_name_leading() -> Result<()> {
        let mut p = StreamParser::new(&b" Content-Type: 3\r\n"[..]);
        let field_name: FieldName = p.parse()?;
        assert_eq!(field_name, FieldName(b"Content-Type".to_vec()));
        assert_eq!(p.complete_buffer(), b": 3\r\n");
    ...
    #[test]
    fn test_field_name_trailing() -> Result<()> {
        let mut p = StreamParser::new(&b"Content-Type \t: 3\r\n"[..]);
        let field_name: FieldName = p.parse()?;
        assert_eq!(field_name, FieldName(b"Content-Type".to_vec()));
        assert_eq!(p.complete_buffer(), b": 3\r\n");
